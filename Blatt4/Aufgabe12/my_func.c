...
char* my_inet_ntoa(struct in_addr in)
{
	...
	int a = big_endian();
	if(a==1)
		snprintf(result,16, "%d.%d.%d.%d", adress_aufteilung[3], adress_aufteilung[2], adress_aufteilung[1], aadress_aufteilung[0]);
	else
		snprintf(result,16, "%d.%d.%d.%d", adress_aufteilung[0], aadress_aufteilung[1], adress_aufteilung[2], adress_aufteilung[3]);

	free(aadress_aufteilung);
